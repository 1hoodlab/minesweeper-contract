    ...
    VRFConsumerBaseV2
{
    ...
    Counters.Counter private _boomUsed;
    ...
    uint256[100] private awards; // store awards
    ...
    function getAward() public view whenPaused returns (uint256[100] memory) {
        return awards;
    }

    function buyTurn(uint8 numberOfTurns)
        external
        payable
        override
        nonReentrant
    {
        uint256 _amount = numberOfTurns * priceOfTurn;
    ...
    function setAwards(uint256[100] memory _awards)
        public
        whenPaused
        onlyOwner
    {
    ...
    function openCell(uint256 key)
        external
        override
        nonReentrant
        whenNotPaused
    {
        require(_boomUsed.current() != 50, "Boom Boom not found");
    ...
    function _handleIncomingFund(uint256 amount) private {
    ...
    function _handleOutGoingFund(address to, uint256 amount) private {
        (bool isSuccess, ) = to.call{value: amount}("");
        require(isSuccess, "Transfer failed: gas error");
    }

    function _handleAward(uint256 key) private {
        if (awards[key] == 0) {
            _boomUsed.increment();
            _deleteTurn();
    ...
    function _handleShuffle(uint256 _seed) private {
        for (uint256 i = 0; i < awards.length; i++) {
            uint256 n = i +
                (uint256(keccak256(abi.encodePacked(_seed))) %
                    (awards.length - i));
            uint256 temp = awards[n];
            awards[n] = awards[i];
            awards[i] = temp;
        }
    }
